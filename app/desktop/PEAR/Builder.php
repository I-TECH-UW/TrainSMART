...
class PEAR_Builder extends PEAR_Common {
	...
	function _build_win32($descfile, $callback = null) {
		...
		$err = $this->_runCommand ( $command, array (
				&$this,
				'msdevCallback' 
		) );
		if (PEAR::isError ( $err )) {
		...
		$regex = '/.*?!IF\s+"\$\(CFG\)"\s+==\s+("' . $pkg->getPackage () . '\s-\s' . $platform . '\s' . $buildtype . '").*?' . '\/out:"(.*?)"/is';
	...
	function _harvestInstDir($dest_prefix, $dirname, &$built_files) {
		...
		while ( ($ent = readdir ( $d )) !== false ) {
			...
			if (is_dir ( $full )) {
				if (! $this->_harvestInstDir ( $dest_prefix . DIRECTORY_SEPARATOR . $ent, $full, $built_files )) {
					$ret = false;
					break;
				}
			} else {
	 ...
	 * Runs "phpize" in the source
	 * directory, but compiles in a temporary directory
	 * (/var/tmp/pear-build-USER/PACKAGE-VERSION).
	 *
	 * @param string|PEAR_PackageFile_v* $descfile
	 *        	path to XML package description file, or
	...
	function build($descfile, $callback = null) {
		...
		$err = $this->_runCommand ( $this->config->get ( 'php_prefix' ) . "phpize" . $this->config->get ( 'php_suffix' ), array (
				&$this,
				'phpizeCallback' 
		) );
		if (PEAR::isError ( $err )) {
		...
		if ($configure_options) {
			foreach ( $configure_options as $o ) {
				...
				list ( $r ) = $this->ui->userDialog ( 'build', array (
						$o ['prompt'] 
				), array (
						'text' 
				), array (
	 ...
	 * program.
	 * Extracts the API numbers used. Ignores other message
	 ...
	 * output.
	 * The command will be run through popen and output is
	 ...
	 *        	
	 * @param mixed $callback
	 *        	(optional) function to use as message
	 *        	callback
	 *        	
