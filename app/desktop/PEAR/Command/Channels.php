...
configuration.' 
	...
	function doUpdateAll($command, $options, $params) {
		...
		foreach ( $channels as $channel ) {
			if ($channel->getName () != '__uri') {
				...
				$err = $this->doUpdate ( 'channel-update', $options, array (
						$channel->getName () 
				) );
				if (PEAR::isError ( $err )) {
	...
	function doInfo($command, $options, $params) {
		...
		$data1 ['data'] ['server'] = array (
				'Name and Server',
				$chan->getName () 
		);
		if ($chan->getAlias () != $chan->getName ()) {
			$data1 ['data'] ['alias'] = array (
					'Alias',
					$chan->getAlias () 
			);
		}
		
		$data1 ['data'] ['summary'] = array (
				'Summary',
				$chan->getSummary () 
		);
		$validate = $chan->getValidationPackage ();
		$data1 ['data'] ['vpackage'] = array (
				'Validation Package Name',
				$validate ['_content'] 
		);
		$data1 ['data'] ['vpackageversion'] = array (
	...
	function doDelete($command, $options, $params) {
		...
		if (count ( $err )) {
			return $this->raiseError ( 'Channel "' . $channel . '" has installed packages, cannot delete' );
		}
	...
	function doAdd($command, $options, $params) {
		...
		if (strpos ( $params [0], '://' )) {
			...
			PEAR::staticPopErrorHandling ();
			if (PEAR::isError ( $loc )) {
				return $this->raiseError ( 'channel-add: Cannot open "' . $params [0] . '" (' . $loc->getMessage () . ')' );
			}
		...
		if ($reg->channelExists ( $channel->getName () )) {
			return $this->raiseError ( 'channel-add: Channel "' . $channel->getName () . '" exists, use channel-update to update entry', PEAR_COMMAND_CHANNELS_CHANNEL_EXISTS );
		}
		...
		if (! $ret) {
			return $this->raiseError ( 'channel-add: adding Channel "' . $channel->getName () . '" to registry failed' );
		}
	...
	function doUpdate($command, $options, $params) {
		...
		if ((! file_exists ( $params [0] ) || is_dir ( $params [0] )) && $reg->channelExists ( strtolower ( $params [0] ) )) {
			...
			if (PEAR::isError ( $contents )) {
				...
				PEAR::staticPushErrorHandling ( PEAR_ERROR_RETURN );
				$contents = $dl->downloadHttp ( 'https://' . $c->getName () . '/channel.xml', $this->ui, $tmpdir, null, $lastmodified );
				PEAR::staticPopErrorHandling ();
				if (PEAR::isError ( $contents )) {
	...
	function doAlias($command, $options, $params) {
		...
		if (count ( $params ) !== 2 || (! empty ( $params [1] ) && $params [1] {0} == '-')) {
			return $this->raiseError ( 'Invalid format, correct is: channel-alias channel alias' );
		}
		...
		if (! $reg->channelExists ( $params [0], true )) {
			...
			if ($reg->isAlias ( $params [0] )) {
				$extra = ' (use "channel-alias ' . $reg->channelName ( $params [0] ) . ' ' . strtolower ( $params [1] ) . '")';
			}
		...
		if ($reg->isAlias ( $params [1] )) {
			return $this->raiseError ( 'Channel "' . $reg->channelName ( $params [1] ) . '" is ' . 'already aliased to "' . strtolower ( $params [1] ) . '", cannot re-alias' );
		}
		
		$chan = &$reg->getChannel ( $params [0] );
		if (PEAR::isError ( $chan )) {
			return $this->raiseError ( 'Corrupt registry?  Error retrieving channel "' . $params [0] . '" information (' . $chan->getMessage () . ')' );
		}
		
		// make it a local alias
		if (! $chan->setAlias ( strtolower ( $params [1] ), true )) {
			return $this->raiseError ( 'Alias "' . strtolower ( $params [1] ) . '" is not a valid channel alias' );
		}
		
		$reg->updateChannel ( $chan );
		$this->ui->outputData ( 'Channel "' . $chan->getName () . '" aliased successfully to "' . strtolower ( $params [1] ) . '"' );
	}
	...
	function doDiscover($command, $options, $params) {
		...
		if ($reg->channelExists ( $channel )) {
			...
			return $this->raiseError ( "A channel alias named \"$channel\" " . 'already exists, aliasing channel "' . $reg->channelName ( $channel ) . '"' );
		}
		
		$this->pushErrorHandling ( PEAR_ERROR_RETURN );
		$err = $this->doAdd ( $command, $options, array (
				'http://' . $channel . '/channel.xml' 
		) );
		$this->popErrorHandling ();
		if (PEAR::isError ( $err )) {
			if ($err->getCode () === PEAR_COMMAND_CHANNELS_CHANNEL_EXISTS) {
				return $this->raiseError ( "Discovery of channel \"$channel\" failed (" . $err->getMessage () . ')' );
			}
	...
	function doLogin($command, $options, $params) {
		...
		list ( $username, $password ) = $this->ui->userDialog ( $command, array (
				'Username',
				'Password' 
		), array (
				'text',
