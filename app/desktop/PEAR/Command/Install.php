...
' 
	...
	function _parseIni($filename) {
		...
		foreach ( $all as $linenum => $line ) {
			...
			if (strtolower ( substr ( $line, 0, strlen ( $zend_extension_line ) ) ) == $zend_extension_line) {
	...
	function doInstall($command, $options, $params) {
			...
			if (strpos ( $param, 'channel://' ) === false && @file_exists ( $param )) {
				...
				if ($exists && $version_compare) {
					if ($this->config->get ( 'verbose' )) {
						$this->ui->outputData ( 'Ignoring installed package ' . $reg->parsedPackageNameToString ( array (
		...
		if (count ( $abstractpackages ) && ! isset ( $options ['force'] )) {
			...
			$abstractpackages = array_map ( array (
					$reg,
		...
		if (isset ( $options ['pretend'] )) {
			foreach ( $downloaded as $package ) {
				$data ['data'] [] = array (
						$reg->parsedPackageNameToString ( $package->getParsedPackage () ) 
				);
			}
		...
		foreach ( $downloaded as $param ) {
			...
			if ($param->getPackageType () == 'extsrc' || $param->getPackageType () == 'extbin' || $param->getPackageType () == 'zendextsrc' || $param->getPackageType () == 'zendextbin') {
				...
				foreach ( $instpkg->getFilelist () as $name => $atts ) {
					...
					if ((strpos ( $pinfo ['basename'], 'php_' ) === 0 && $pinfo ['extension'] == 'dll') ||
                          // most unices
                          $pinfo || 
 ['extension'] == 'so' ||
                          // hp-ux
 ...
 ['extension'] == 'sl') {
				...
				if (count ( $binaries )) {
					foreach ( $binaries as $pinfo ) {
						PEAR::staticPushErrorHandling ( PEAR_ERROR_RETURN );
						$ret = $this->enableExtension ( array (
								$pinfo [0] 
						), $param->getPackageType () );
						PEAR::staticPopErrorHandling ();
						if (PEAR::isError ( $ret )) {
			...
			if ($this->config->get ( 'verbose' ) > 0) {
				...
				$label = $reg->parsedPackageNameToString ( array (
						'channel' => $chan,
						'package' => $param->getPackage (),
						'version' => $param->getVersion () 
				) );
				$out = array (
						'data' => "$command ok: $label" 
				);
			...
			if ($deps) {
				if (isset ( $deps ['group'] )) {
					...
					$extrainfo [] = $param->getPackage () . ': To install optional features use "pear install ' . $reg->parsedPackageNameToString ( array (
							'package' => $param->getPackage (),
	...
	function doUninstall($command, $options, $params) {
		...
		foreach ( $params as $pkg ) {
			...
			if ($info === null && ($channel == 'pear.php.net' || $channel == 'pecl.php.net')) {
			...
			if ($info === null) {
				...
				if ($installedbinary = $info->getInstalledBinary ()) {
					$this->ui->log ( 'adding binary package ' . $reg->parsedPackageNameToString ( array (
							'channel' => $channel,
		...
		foreach ( $params as $pkg ) {
			...
			if ($err = $this->installer->uninstall ( $pkg, $options )) {
				...
				if ($pkg->getPackageType () == 'extsrc' || $pkg->getPackageType () == 'extbin' || $pkg->getPackageType () == 'zendextsrc' || $pkg->getPackageType () == 'zendextbin') {
					if ($instbin = $pkg->getInstalledBinary ()) {
 ...
 ['extension'] == 'sl') {
					...
					if (count ( $binaries )) {
						foreach ( $binaries as $pinfo ) {
							PEAR::staticPushErrorHandling ( PEAR_ERROR_RETURN );
							$ret = $this->disableExtension ( array (
									$pinfo [0] 
							), $pkg->getPackageType () );
							PEAR::staticPopErrorHandling ();
							if (PEAR::isError ( $ret )) {
								$extrainfo [] = $ret->getMessage ();
								if ($pkg->getPackageType () == 'extsrc' || $pkg->getPackageType () == 'extbin') {
									$exttype = 'extension';
								} else {
									ob_start ();
									phpinfo ( INFO_GENERAL );
								...
								$this->ui->outputData ( 'Unable to remove "' . $exttype . '=' . $pinfo [1] ['basename'] . '" from php.ini', $command );
							} else {
								$this->ui->outputData ( 'Extension ' . $pkg->getProvidesExtension () . ' disabled in php.ini', $command );
							}
						}
					}
				}
				...
				if (! isset ( $options ['offline'] ) && is_object ( $savepkg ) && defined ( 'PEAR_REMOTEINSTALL_OK' )) {
					if ($this->config->isDefinedLayer ( 'ftp' )) {
						$this->installer->pushErrorHandling ( PEAR_ERROR_RETURN );
	...
	function doBundle($command, $options, $params) {
		...
		$result = &$downloader->download ( array (
				$params [0] 
		) );
			...
			if (isset ( $latestReleases [$channel] [$name_lower] )) {
				...
				if ($command == 'upgrade-all') {
					$this->ui->outputData ( array (
							'data' => 'Will upgrade ' . $reg->parsedPackageNameToString ( $package ) 
					), $command );
				}
				$ret [] = $package;
			}
