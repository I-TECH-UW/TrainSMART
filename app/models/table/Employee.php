...
link_employee_facility.hiv_fte_related, 
 facility.location_id, dsd_model_id, dsd_team_id, mechanism_option_id
...
WHERE (employee_id = $employee_id) order by link_employee_facility.id"; //#387
	...
	//TA:#438
	public static function removeAllSites($employee_id){
...
//TA:#224, TA:#416, TA:#464
	public static function saveSites ( $employee_id, $site_id, $dsd_model_id, $dsd_team_id, $hiv_related_fte, $mech_option_id){
	Employee::removeSites($employee_id);
	$linkTable = new ITechTable ( array ('name' => 'link_employee_facility' ) );
	    try {
	    $row = $linkTable->createRow(array('employee_id' => $employee_id, 'facility_id' => $site_id, 'dsd_model_id' => $dsd_model_id, 'dsd_team_id' => $dsd_team_id, 'hiv_fte_related' => $hiv_related_fte, 'mechanism_option_id' => $mech_option_id));
	        $row->save();
	    } catch(Exception $e) {
	    error_log($e);
	    return false;
