...
//         if($ld['id'] === $rd['id'] && !($ld['id_tutor'] === $rd['id_tutor'] && $ld['id_language'] === $rd['id_language'])){
...
//         }
    ...
    public function fetchFieldMatch($ld)
    {
        ...
        $row = $this->getRightTable()->fetchRow("(id_tutor=" . @$ld->id_tutor . " AND id=" . @$ld->id . " AND id_language=" . @$ld->id_language . ")");
        if ($row) {
            return $row;
        }
        return null;
    ...
    public function deleteMember($right_id, $commit=false) {
        ...
        $this->log = $this->log . "DELETE: id_tutor=" . $rItem->id_tutor . ", id_language=" . $rItem->id_language .  "\n";
       
    }

    public function fetchHardDeletes($path, $field){
       // return null; // to avoid removing data entered by others
    ...
    public function insertMember($left_id, $path=null, $field=null, $commit = false) {
        ...
        try {
            ...
            $rows = $rTable->fetchAll("id_tutor=" . $lItem->id_tutor . " and id_language=" . $lItem->id_language);
            ...
            $this->log = $this->log . "INSERT: id_tutor=" . $lItem->id_tutor . ", id_language=" . $lItem->id_language . "\n";
            if(!$new_tutor_link) {
                $this->log = $this->log . "INSERT ERROR: id_tutor=" . $lItem->id_tutor . ", id_language=" . $lItem->id_language . "\n";
