...
*/
	...
	public function trainingReport() {
		...
		if ($criteria ['go']) {
			...
			if ($criteria ['doCount'] || $criteria ['doName']) {
				$sql .= ' FROM (SELECT distinct training.*, training_location.training_location_name, person.primary_qualification_option_id, person_to_training_viewing_loc_option.location_phrase as location_phrase,'.implode(',',$field_name);
			    
			    $sql .= ', person.id as person_id, tto.training_title_phrase AS training_title, person.custom_3 as person_custom_3, person.custom_4 as person_custom_4, person.custom_5 as person_custom_5 ' .
			    '		 , ptao.award_phrase, ptao.id as ptao_id '.			
				'         FROM training ' .
				'         LEFT JOIN training_title_option tto ON (`training`.training_title_option_id = tto.id)' .
				
				'		  inner JOIN person_to_training ON training.id = person_to_training.training_id '.
				'		  LEFT JOIN person_to_training_award_option as ptao ON ptao.id = person_to_training.award_id '.
				'		  inner JOIN person ON person.id = person_to_training.person_id '.
				'		  inner JOIN facility ON person.facility_id = facility.id '.
			...
			if ($criteria ['person_to_training_award_option_id']) {
				$where [] = 'pt.ptao_id = ' . $criteria['person_to_training_award_option_id'];
			}
			...
			if ($criteria ['doCount']) {
				...
				if ($criteria ['showTrainingCompletion']) $groupBy []=  '  pt.award_phrase';
	...
	public function trainingUnknownAction() {
		...
		if ($criteria ['go']) {
			...
			if ($criteria ['showTrainingCompletion'] || $criteria ['person_to_training_award_option_id']) {
				$sql .= ' LEFT JOIN person_to_training_award_option as ptao ON ptao.id = pt.person_to_training_award_option_id ';
			}
			...
			if ($criteria ['doCount']) {
				...
				if ($criteria ['showPrimaryLanguage'])   $groupBy [] = 'pt.training_primary_language_option_id';
				if ($criteria ['showSecondaryLanguage']) $groupBy [] = 'pt.training_secondary_language_option_id';
				if ($criteria ['showTrainingCompletion']) $groupBy [] = 'pt.person_to_training_award_option_id';
				if ($criteria ['showFunding'])           $groupBy [] = 'tfund.training_funding_option_id';
...
*/
			...
			$sql .= ' FROM (';
			$sql .= 'SELECT distinct training.*, person.facility_id as "facility_id", person.id as "person_id", person.last_name, IFNULL(suffix_phrase, ' . "' '" . ') as suffix_phrase, ';
			...
			$sql .= ', ptao.award_phrase, ptao.id as ptao_id ';
			/*
			$sql .= ' FROM training ';
			$sql .= '    LEFT JOIN training_title_option tto ON training.training_title_option_id = tto.id ';
			...
			$sql .= '    left join person_to_training on person_to_training.training_id = training.id and person_to_training.person_id = person.id ';
			...
			*/
			// gnr: Mar.10.14 select from  person_to_training not training
			$sql .= ' from person_to_training  ';
			
			$sql .= ' left join training on person_to_training.training_id = training.id ';
			$sql .= ' left join person on person.id = person_to_training.person_id ';
			$sql .= ' left join person_suffix_option suffix on person.suffix_option_id = suffix.id ';
			$sql .= ' left join person_to_training_award_option as ptao on ptao.id = person_to_training.award_id ';
			$sql .= ' left join training_title_option tto on training.training_title_option_id = tto.id ';
			$sql .= ' left join facility on person.facility_id = facility.id ';
			$sql .= ' left join training_to_trainer on training_to_trainer.training_id = training.id and training_to_trainer.trainer_id = person.id ';
			...
			foreach ( $rowArray as $row ) {
				...
				foreach ( $persons as $r ) {
					...
					if ( $add_additional ) {
						...
						if ( $this->setting ( 'display_region_b' ) ) $body_fields[] = $locations[$region_ids['province_id']]['name'];
