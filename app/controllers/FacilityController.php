	...
	protected function validateAndSave($facilityRow, $checkName = true) {
		...
		if ($status->hasError ()) {
			...
			if ($location_id) {
				...
				$facilityRow->custom_1               = $this->getSanParam ( 'custom_1' );
  ...
  and inner_facility.id = facility.id ) as facility_sponsor_phrase , 
		...
		if ( $filename )
		{
			...
			while ($row = $this->_csv_get_row($filename) )
			{
				...
				if ( $countValidFields ) {
					...
					//required
					if( empty($values['facility_name']) ) { $errs[] = t('Error adding facility, Facility name cannot be empty.'); continue; }
					if( empty($values['address_1']) ) { $values['address_1'] = ''; }
					if( empty($values['address_2']) ) { $values['address_2'] = ''; }
					//locations
					...
					if (! $location_id )
					{
						for ($i=0; $i <= $num_location_tiers; $i++) { // insert/find locations
							$r = 11 + $i; // first location field in csv row // could use this too: $values[t('Region A (Province)')]
		...
		if ( $filename )
		{
			...
			while ($row = $this->_csv_get_row($filename) )
			{
				...
				if ( $countValidFields ) {
					...
					if ( isset($values['timestamp_updated']) ){ unset($values['timestamp_updated']); }
					//required
					if( empty($values['training_location_name']) ) { $errs[] = t('Error adding training location, training location name cannot be empty.'); }
					//locations
					$num_location_tiers = $this->setting('num_location_tiers');
					$bSuccess = true;
					$location_id = null;
	...
	*/
	public function importLocationTemplateAction() {
		...
		$sorted = array (array ('training_location_name' => 'Sample Location'));
